...
本研究では、こうした背景を踏まえ、実際に視聴・研究可能な*調理系動画チャンネル*を複数選定した。動画の出典は主にYouTubeおよびBiliBiliの2つの動画投稿プラットフォームである。

YouTubeはアメリカ発のグローバルな動画共有サービスであり、世界中の個人・団体がコンテンツを配信している。特に中国語圏のユーザーによるチャンネルも数多く存在し、**海外に居住する中国語話者**による料理動画も豊富に見られる。
...
これらを放置すると、コーパスとしての構造的な一貫性が損なわれ、後続処理（動詞抽出、クラスタリング）にも悪影響を及ぼす。そのため、本研究では**ローカル大規模言語モデル（LLM）である Qwen3-14B**を用いて、文整形（cleaning）を自動化する手法を導入した。

Qwen3-14Bは、Alibaba Group によって開発された中規模のオープンウェイト言語モデルであり、14.8億パラメータという比較的軽量な構成ながら、推論精度と実行効率のバランスに優れる点が特徴である。多言語対応の設計で、中国語と英語に特に高い性能を発揮し、**日本国内の研究環境でもローカル実行可能なLLMの選択肢**として注目されている。
具体的には、OllamaでQ4_K_Mとして量子化されたバージョンを使用した。

整形処理では、Qwen3-14B に「重複文の除去」「無意味な繰り返しの削除」などのタスクを指示するプロンプトを与え、**句読点の整備を含む自然な文単位への整理を実施**した。なお、**語彙の言い換えや再構成は明示的に禁止し、コーパスの忠実性を保持する設計**とした。
