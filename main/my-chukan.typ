...
その一方,食に関する自然言語処理（NLP）の研究は,ニュースや商品レビュー,医療分野と比べると,まだ十分に整備された大規模なコーパスやリソースが限られている.特に,調理行為に関連する動詞表現の用法や手続き的知識の構造化といった観点から，動画音声・字幕を統合的に解析する研究は極めて少ないのが現状である.
...
本稿ではまず、対象動画の選定と収集手順について述べた後、ASR（Automatic Speech Recognition）による文字起こしに先立ち,音声区間の抽出やフォーマット変換などの前処理を行い,そしてLLMによる整形処理,HanLPによるトークナイズといった一連のコーパス構築工程について詳述する。さらに、他の料理コーパス（例：XiaChuFang Recipe Corpus）との比較を通じて、本研究の位置付けを明確にし、現状の課題と将来的な言語応用の可能性について考察する。


= 先行研究

近年、食分野における大規模言語モデル（LLM）の応用が進んでいる。たとえば FoodSky(@foodsky)は、調理と栄養に関する知識を深く理解し、自然言語で助言・生成を行うことを目指した中国語特化のLLMである。FoodSkyでは、命令文ベースのデータを収集・整備した大規模食コーパス「FoodEarth」を構築し、多段階のデータ整備パイプラインによって、Webサイトや電子書籍などの多様な情報源から得られるノイズの多いデータを高品質に変換している。また、TS3MおよびHTRAGといった独自のモデル設計によって、細粒度な食セマンティクスと文化的多様性に対応可能な言語生成を実現している。

他方、料理レシピの自然言語データとしては、「下厨房（XiaChuFang）」という中国の料理共有サイトから収集された大規模レシピデータセット「XiaChuFang Recipe Corpus」(@XiaChuFang)も存在する。このコーパスには150万件以上のレシピが含まれ、それぞれが料理名、食材、調理工程、検索キーワード、記述的説明などの構造化情報を備えている。家庭的かつ実用的な口語表現が多く含まれており、実際の調理場面で使われる語彙や表現の分析に適している。
...
#table(
  ...
  [形式], [Q&A形式（instruction-based）], [料理名、食材、手順、説明の構造化], [メタ情報、テキスト、動詞・工程データベース],
...
近年、料理や生活に関する情報の発信手段として、動画共有サイトの活用が一般化している。特に料理動画は、視覚・聴覚・言語が同時に伝達されるマルチモーダルな情報資源として、自然言語処理や意味解析の研究対象としても注目されつつある。

本研究では、こうした背景を踏まえ、実際に視聴・研究可能な調理系動画チャンネルを複数選定した。動画の出典は主にYouTubeおよびBiliBiliの2つの動画投稿プラットフォームである。

YouTubeはアメリカ発のグローバルな動画共有サービスであり、世界中の個人・団体がコンテンツを配信している。特に中国語圏のユーザーによるチャンネルも数多く存在し、海外に居住する中国語話者による料理動画も豊富に見られる。

一方、BiliBili（哔哩哔哩）は中国国内で人気の動画プラットフォームであり、若年層を中心にニッチかつ専門的な動画が投稿・視聴される傾向がある。コメントの弾幕文化や字幕支援機能の充実など、言語資料としての利点も多く、本研究ではBiliBiliも対象とした。

以上を踏まえ、YouTubeでは主に「阿朝哥美食」「美食作家王刚」「尚食厨房」などの登録者数・投稿数ともに安定しているチャンネルを選出し、BiliBiliでは「老东北美食」を収集対象とした。
...
本研究では、調理動画から音声・字幕・構造的な付帯情報を高精度に取得するため、yt-dlp（YouTube Download Plus）というオープンソースツールを採用した。/*これは、従来の `youtube-dl` をベースに派生・発展したコマンドラインツールであり、特に**高機能・高速性・サイト対応力**において優れている。*/

yt-dlpはYouTubeをはじめ、VimeoやBiliBiliなど1,000以上の動画共有サイトに対応しており、以下のような多彩な機能を備えている：

/* 実際の設定を説明する簡単な説明のみ */
- 動画・音声ファイルのダウンロード（フォーマット・解像度指定も可能）
- 字幕ファイル（`.srt`, `.vtt`）の取得と埋め込み
- JSON形式のメタデータの抽出（動画タイトル、URL、投稿日、チャンネル名、再生数など）
...
Whisperは、世界中の複数言語の音声データで学習された大規模な自動音声認識（ASR）モデルであり、特に標準中国語（普通话）に対して非常に高い認識精度を持つ。本研究では、現在利用可能な中でも最も大規模で精度の高い`large-v3`モデルを選定した。
/*
このモデルは約1,550Mパラメータを持ち、以下のような特徴を備えている：
...
ただし、強い方言や背景音が多い動画、話速の早い場面では一部精度が低下する傾向も確認された。これらの課題については、後述するローカルLLMを用いた整形処理によって補完することで、実用的なコーパス構築に資する文字列品質を担保している。

以上のように、Whisperの導入により、本研究は調理音声の大規模な自動文字起こしを可能とし、マルチモーダルコーパスの基盤構築に大きく貢献した。

また、中国語に特化した音声認識モデルとしては、中国・アリババ社が開発した FunASR @gao2023funasr も存在する。FunASRは、標準語に加えて北京方言などの地域変種にも対応した軽量モデル（SenseVoiceSmall）/*を搭載し、句読点挿入や音声区間検出（VAD）などを標準機能として備える。*/

一方で、Whisperは多言語に対応しつつ、中国語においても非常に高い精度を持ち、特に句読点や構文の安定感、雑音耐性において実用上の信頼性が高いことから、本研究ではまずWhisperを主要なASRエンジンとして採用した。

今後の課題としては、WhisperとFunASRの出力結果を比較分析し、具体的にどちらが調理動画に適しているかを評価することが挙げられる。特に、方言や口語的表現が頻出する場面において、両モデルの出力傾向や誤認識のパターンを可視化・評価していく必要がある。
...
Qwen3-14Bは、Alibaba Group によって開発された中規模のオープンウェイト言語モデルであり、14.8億パラメータという比較的軽量な構成ながら、推論精度と実行効率のバランスに優れる点が特徴である。多言語対応の設計で、中国語と英語に特に高い性能を発揮し、日本国内の研究環境でもローカル実行可能なLLMの選択肢として注目されている。
具体的には、OllamaでQ4_K_Mとして量子化されたバージョンを使用した。
...
現時点では、この整形処理の精度や妥当性に関する定量的な評価（例：文区切りの正確性や冗長率の変化）は行われておらず、今後の検証課題とする。今後は、正規表現やルールベース手法と比較しながら、LLMによる整形の意義と限界を明らかにしていく予定である。
...
本研究では、中国語向けの高性能NLPライブラリである HanLP(@hanlp)を用いて、整形済みテキストに対する分かち書きおよび品詞タグ付け処理を行う準備を進めている。

HanLP は中国語処理を中心に設計されたオープンソースの総合NLPツールキットであり、以下のような特徴を持つ：
...
== Chinese\_Video\_Corpus構築における課題
...
#figure(
  image("conllu-tree.svg", width: 100%), 
